foreach(REQUIRED ARM_BUNDLE X86_BUNDLE ARM_AU_BUNDLE X86_AU_BUNDLE
                 PACKAGE_DIR RESOURCE_README INSTALLER)
  if(NOT DEFINED ${REQUIRED})
    message(FATAL_ERROR "${REQUIRED} is required")
  endif()
endforeach()
if(NOT EXISTS "${ARM_BUNDLE}" OR NOT EXISTS "${X86_BUNDLE}" OR
   NOT EXISTS "${ARM_AU_BUNDLE}" OR NOT EXISTS "${X86_AU_BUNDLE}")
  message(FATAL_ERROR "Both arm64 and x86_64 VST3 and AU bundles must exist")
endif()

set(PLUGIN_NAME "Ensoniq EPS-16 Plus")
set(STAGE "${PACKAGE_DIR}/${PLUGIN_NAME}.stage")
set(FINAL "${PACKAGE_DIR}/${PLUGIN_NAME}.vst3")
set(AU_STAGE "${PACKAGE_DIR}/${PLUGIN_NAME}.au-stage")
set(AU_FINAL "${PACKAGE_DIR}/${PLUGIN_NAME}.component")
set(RESOURCES "${PACKAGE_DIR}/EPS_files")
if(NOT DEFINED ARCHIVE_NAME)
  set(ARCHIVE_NAME "Ensoniq-EPS-16-Plus-macOS-universal.zip")
endif()
...
set(ARM_AU_BINARY "${ARM_AU_BUNDLE}/Contents/MacOS/${PLUGIN_NAME}")
set(X86_AU_BINARY "${X86_AU_BUNDLE}/Contents/MacOS/${PLUGIN_NAME}")
set(AU_STAGE_BINARY "${AU_STAGE}/Contents/MacOS/${PLUGIN_NAME}")
set(INSTALL_COMMAND "${PACKAGE_DIR}/Install EPS-16 Plus.command")
set(PACKAGE_ITEMS "${PLUGIN_NAME}.vst3" "${PLUGIN_NAME}.component" "EPS_files"
...
run_checked(/usr/bin/ditto --norsrc --noextattr --noqtn --noacl
            "${X86_AU_BUNDLE}" "${AU_STAGE}")
run_checked(/usr/bin/lipo -create "${X86_AU_BINARY}" "${ARM_AU_BINARY}"
            -output "${AU_STAGE_BINARY}")
run_checked(/usr/bin/xattr -cr "${AU_STAGE}")
run_checked(/usr/bin/codesign --force --deep --sign - "${AU_STAGE}")
run_checked(/usr/bin/codesign --verify --deep --strict --verbose=2 "${AU_STAGE}")
...
execute_process(COMMAND /usr/bin/lipo -archs "${AU_STAGE_BINARY}"
                OUTPUT_VARIABLE AU_ARCHS OUTPUT_STRIP_TRAILING_WHITESPACE)
if(NOT AU_ARCHS MATCHES "arm64" OR NOT AU_ARCHS MATCHES "x86_64")
  message(FATAL_ERROR "Universal AU has unexpected architectures: ${AU_ARCHS}")
endif()
...
set(X86_AU_THIN "${PACKAGE_DIR}/verify-au-x86_64")
set(ARM_AU_THIN "${PACKAGE_DIR}/verify-au-arm64")
run_checked(/usr/bin/lipo "${AU_STAGE_BINARY}" -thin x86_64
            -output "${X86_AU_THIN}")
run_checked(/usr/bin/lipo "${AU_STAGE_BINARY}" -thin arm64
            -output "${ARM_AU_THIN}")
execute_process(COMMAND /usr/bin/otool -l "${X86_AU_THIN}"
                OUTPUT_VARIABLE X86_AU_LOAD_COMMANDS)
execute_process(COMMAND /usr/bin/otool -l "${ARM_AU_THIN}"
                OUTPUT_VARIABLE ARM_AU_LOAD_COMMANDS)
if(NOT X86_AU_LOAD_COMMANDS MATCHES "version 10\\.13")
  message(FATAL_ERROR "x86_64 AU slice does not target macOS 10.13")
endif()
if(NOT ARM_AU_LOAD_COMMANDS MATCHES "minos 11\\.0" AND
   NOT ARM_AU_LOAD_COMMANDS MATCHES "version 11\\.0")
  message(FATAL_ERROR "arm64 AU slice does not target macOS 11")
endif()
file(REMOVE "${X86_AU_THIN}" "${ARM_AU_THIN}")

file(RENAME "${STAGE}" "${FINAL}")
file(RENAME "${AU_STAGE}" "${AU_FINAL}")
